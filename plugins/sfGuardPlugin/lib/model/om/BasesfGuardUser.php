...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array MawsParser[] Collection to store aggregation of MawsParser objects.
	 */
	protected $collMawsParsers;

	/**
	 * @var        Criteria The criteria used to select the current contents of collMawsParsers.
	 */
	private $lastMawsParserCriteria = null;

	/**
	 * @var        array MawsThread[] Collection to store aggregation of MawsThread objects.
	 */
	protected $collMawsThreads;

	/**
	 * @var        Criteria The criteria used to select the current contents of collMawsThreads.
	 */
	private $lastMawsThreadCriteria = null;

	/**
	 * @var        array MawsPage[] Collection to store aggregation of MawsPage objects.
	 */
	protected $collMawsPages;

	/**
	 * @var        Criteria The criteria used to select the current contents of collMawsPages.
	 */
	private $lastMawsPageCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collMawsParsers = null;
			$this->lastMawsParserCriteria = null;

			$this->collMawsThreads = null;
			$this->lastMawsThreadCriteria = null;

			$this->collMawsPages = null;
			$this->lastMawsPageCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collMawsParsers !== null) {
				foreach ($this->collMawsParsers as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collMawsThreads !== null) {
				foreach ($this->collMawsThreads as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collMawsPages !== null) {
				foreach ($this->collMawsPages as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getMawsParsers() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addMawsParser($relObj->copy($deepCopy));
				}
			}

			foreach ($this->getMawsThreads() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addMawsThread($relObj->copy($deepCopy));
				}
			}

			foreach ($this->getMawsPages() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addMawsPage($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collMawsParsers collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 ...
	 * @see        addMawsParsers()
	 */
	public function clearMawsParsers()
	{
		$this->collMawsParsers = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collMawsParsers collection (array).
	 *
	 * By default this just sets the collMawsParsers collection to an empty array (like clearcollMawsParsers());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initMawsParsers()
	{
		$this->collMawsParsers = array();
	}

	/**
	 * Gets an array of MawsParser objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related MawsParsers from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array MawsParser[]
	 * @throws     PropelException
	 */
	public function getMawsParsers($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collMawsParsers === null) {
			if ($this->isNew()) {
			   $this->collMawsParsers = array();
				...
				$criteria->add(MawsParserPeer::OWNER_ID, $this->id);

				MawsParserPeer::addSelectColumns($criteria);
				$this->collMawsParsers = MawsParserPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				MawsParserPeer::addSelectColumns($criteria);
				if (!isset($this->lastMawsParserCriteria) || !$this->lastMawsParserCriteria->equals($criteria)) {
					$this->collMawsParsers = MawsParserPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastMawsParserCriteria = $criteria;
		return $this->collMawsParsers;
	}

	/**
	 * Returns the number of related MawsParser objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related MawsParser objects.
	 * @throws     PropelException
	 */
	public function countMawsParsers(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collMawsParsers === null) {
			if ($this->isNew()) {
				...
				$count = MawsParserPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastMawsParserCriteria) || !$this->lastMawsParserCriteria->equals($criteria)) {
					...
					$count = count($this->collMawsParsers);
				}
			} else {
				$count = count($this->collMawsParsers);
			}
	...
	/**
	 * Method called to associate a MawsParser object to this object
	 * through the MawsParser foreign key attribute.
	 *
	 * @param      MawsParser $l MawsParser
	 * @return     void
	 * @throws     PropelException
	 */
	public function addMawsParser(MawsParser $l)
	{
		if ($this->collMawsParsers === null) {
			$this->initMawsParsers();
		}
		if (!in_array($l, $this->collMawsParsers, true)) { // only add it if the **same** object is not already associated
			array_push($this->collMawsParsers, $l);
			$l->setsfGuardUser($this);
		}
	}

	/**
	 * Clears out the collMawsThreads collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 ...
	 * @see        addMawsThreads()
	 */
	public function clearMawsThreads()
	{
		$this->collMawsThreads = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collMawsThreads collection (array).
	 *
	 * By default this just sets the collMawsThreads collection to an empty array (like clearcollMawsThreads());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initMawsThreads()
	{
		$this->collMawsThreads = array();
	}

	/**
	 * Gets an array of MawsThread objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related MawsThreads from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array MawsThread[]
	 * @throws     PropelException
	 */
	public function getMawsThreads($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collMawsThreads === null) {
			if ($this->isNew()) {
			   $this->collMawsThreads = array();
				...
				$criteria->add(MawsThreadPeer::OWNER_ID, $this->id);

				MawsThreadPeer::addSelectColumns($criteria);
				$this->collMawsThreads = MawsThreadPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				MawsThreadPeer::addSelectColumns($criteria);
				if (!isset($this->lastMawsThreadCriteria) || !$this->lastMawsThreadCriteria->equals($criteria)) {
					$this->collMawsThreads = MawsThreadPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastMawsThreadCriteria = $criteria;
		return $this->collMawsThreads;
	}

	/**
	 * Returns the number of related MawsThread objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related MawsThread objects.
	 * @throws     PropelException
	 */
	public function countMawsThreads(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collMawsThreads === null) {
			if ($this->isNew()) {
				...
				$count = MawsThreadPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastMawsThreadCriteria) || !$this->lastMawsThreadCriteria->equals($criteria)) {
					...
					$count = count($this->collMawsThreads);
				}
			} else {
				$count = count($this->collMawsThreads);
			}
	...
	/**
	 * Method called to associate a MawsThread object to this object
	 * through the MawsThread foreign key attribute.
	 *
	 * @param      MawsThread $l MawsThread
	 * @return     void
	 * @throws     PropelException
	 */
	public function addMawsThread(MawsThread $l)
	{
		if ($this->collMawsThreads === null) {
			$this->initMawsThreads();
		}
		if (!in_array($l, $this->collMawsThreads, true)) { // only add it if the **same** object is not already associated
			array_push($this->collMawsThreads, $l);
			$l->setsfGuardUser($this);
		}
	}

	/**
	 * Clears out the collMawsPages collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 ...
	 * @see        addMawsPages()
	 */
	public function clearMawsPages()
	{
		$this->collMawsPages = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collMawsPages collection (array).
	 *
	 * By default this just sets the collMawsPages collection to an empty array (like clearcollMawsPages());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initMawsPages()
	{
		$this->collMawsPages = array();
	}

	/**
	 * Gets an array of MawsPage objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related MawsPages from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array MawsPage[]
	 * @throws     PropelException
	 */
	public function getMawsPages($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collMawsPages === null) {
			if ($this->isNew()) {
			   $this->collMawsPages = array();
				...
				$criteria->add(MawsPagePeer::OWNER_ID, $this->id);

				MawsPagePeer::addSelectColumns($criteria);
				$this->collMawsPages = MawsPagePeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				MawsPagePeer::addSelectColumns($criteria);
				if (!isset($this->lastMawsPageCriteria) || !$this->lastMawsPageCriteria->equals($criteria)) {
					$this->collMawsPages = MawsPagePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastMawsPageCriteria = $criteria;
		return $this->collMawsPages;
	}

	/**
	 * Returns the number of related MawsPage objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related MawsPage objects.
	 * @throws     PropelException
	 */
	public function countMawsPages(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collMawsPages === null) {
			if ($this->isNew()) {
				...
				$count = MawsPagePeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastMawsPageCriteria) || !$this->lastMawsPageCriteria->equals($criteria)) {
					...
					$count = count($this->collMawsPages);
				}
			} else {
				$count = count($this->collMawsPages);
			}
	...
	/**
	 * Method called to associate a MawsPage object to this object
	 * through the MawsPage foreign key attribute.
	 *
	 * @param      MawsPage $l MawsPage
	 * @return     void
	 * @throws     PropelException
	 */
	public function addMawsPage(MawsPage $l)
	{
		if ($this->collMawsPages === null) {
			$this->initMawsPages();
		}
		if (!in_array($l, $this->collMawsPages, true)) { // only add it if the **same** object is not already associated
			array_push($this->collMawsPages, $l);
			$l->setsfGuardUser($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collMawsParsers) {
				foreach ((array) $this->collMawsParsers as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collMawsThreads) {
				foreach ((array) $this->collMawsThreads as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collMawsPages) {
				foreach ((array) $this->collMawsPages as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
